-- Repair partial 0023 run: date_payments existed without paystack_reference.
-- Safe when date_payments has no production rows yet (Phase 1).
...
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE 'Dropped incomplete date_payments table (missing paystack_reference)';
  END IF;
END $$;
