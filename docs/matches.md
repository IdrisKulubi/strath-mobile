...
| Variable | Purpose |
|----------|---------|
| `MIN_CANDIDATE_MATCH_SCORE` | Base minimum compatibility (0–100). |
| `MIN_CANDIDATE_MATCH_SCORE_FLOOR` | Hard floor after relax. |
| `MAX_CANDIDATE_QUEUE_SIZE` | Max rows per batch (1 live + rest queued). |
| `CANDIDATE_PAIR_EXPIRY_MINUTES` | Active card lifetime. |
| `MATCH_*` | Wait / sparse / imbalance / relax steps (fairness). |
| `MATCH_IMBALANCE_RELAX_ENABLED` | Optional; default off in behavior unless set `true`. |
