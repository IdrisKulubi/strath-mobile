...
## Model
Additive q_ tables contain immutable published question versions/options/categories, answers (private by default), per-user answer revision and progress, private DOB and explicit discovery preferences, `q_profile_decisions`, canonical `q_connections` referencing existing `matches`, privacy-safe connection events, and the revision-keyed score cache. The pair row exists from the first decision and is transaction-locked so simultaneous reciprocal likes cannot miss each other. Do not infer partner preferences from legacy profile fields. No legacy tables are deleted.
...
Apply explicit migrations `0038_questionnaire_matching.sql`, `0039_questionnaire_discovery.sql` and `0040_questionnaire_connections.sql` through the checked-in migration runner, then seed the catalogue. Rehearse on an isolated database first. Backfill only listable legacy mutual matches with a valid existing conversation and no blocked pair or inactive duplicate; retain IDs/messages. Do not reactivate cancelled/expired/unmatched connections. The `q_` tables own the new lifecycle. Keep all legacy booking/payment operations intact. Run the reconciliation dry run and apply step before enabling flags. No production migration is part of local validation.
