...
- [x] Advance single choices with feedback; use Continue for partner multi-select. The person's own answer remains in the payload but is hidden on the partner beat. Select all uses the same checkbox row, existing option IDs, and can be reversed.
