...
test('brief mutation ids are stable for retries and change with version or operation', () => {
  const input = {
    baseVersion: 2,
    operations: [{ type: 'add' as const, category: 'values' as const, value: 'kind' }],
  };
  ...
  assert.notEqual(
    buildMatchmakerBriefMutationId(input),
    buildMatchmakerBriefMutationId({
      ...input,
      operations: [{ ...input.operations[0], value: 'honest' }],
    }),
  );
});
