...
import { batchNumber, batchProgress, createInteractionGate, IMPORTANCE_CHOICES, isBatchMilestone, isCompleteQuestionDraft, isUsableDraft, nextQuestion, partnerChoiceLayout, QUESTION_BEAT_COUNT, resumePartnerDraft, toggleAcceptable, toggleAllAcceptable } from './questionnaire-flow.ts';
...
test('partner choices can differ from the personal answer and select-all reverses', () => {
  const choices = ['throughout', 'daily', 'sometimes'];
  const all = toggleAllAcceptable(choices, []);
  assert.deepEqual(all, choices);
  assert.deepEqual(toggleAllAcceptable(choices, all), []);
  assert.deepEqual(toggleAcceptable('throughout', all), ['daily', 'sometimes']);
  assert.deepEqual(toggleAcceptable('daily', ['throughout']), ['throughout', 'daily']);
  assert.equal(isCompleteQuestionDraft(['throughout', 'daily'], 'throughout', ['daily'], 10, ''), true);
  assert.equal(isCompleteQuestionDraft(['throughout', 'daily'], 'throughout', [], 10, ''), false);
});

test('children partner choices show Yes, No, and Unsure regardless of the personal answer', () => {
  const options = [
    { id: '0', label: 'Yes' },
    { id: '1', label: 'No' },
    { id: '2', label: 'Unsure' },
  ...
  const layout = partnerChoiceLayout(options, '3');
  assert.deepEqual(layout.visibleOptions.map(({ label }) => label), ['Yes', 'No', 'Unsure']);
  assert.equal(layout.showSelectAll, true);
  assert.equal(isCompleteQuestionDraft(options.map(({ id }) => id), '1', ['0'], 10, ''), true);
  assert.equal(isCompleteQuestionDraft(options.map(({ id }) => id), '1', ['1'], 10, ''), true);
  assert.equal(isCompleteQuestionDraft(options.map(({ id }) => id), '1', ['0', '2'], 10, ''), true);
  assert.equal(partnerChoiceLayout(options.slice(0, 2), null).showSelectAll, false);
});

test('a paused beat can restore and Back can retain choices without treating the draft as saved', () => {
  const draft = { userId: 'a', questionId: 'q001:1', revision: 2, answer: 'throughout', acceptable: ['throughout', 'daily'], partnerChoicesExplicit: true, weight: 10, visible: false, explanation: '', beat: 3, weightChosen: true, visibilityChosen: false };
  assert.equal(isUsableDraft(draft, { userId: 'a', questionId: 'q001:1', revision: 2, optionIds: ['throughout', 'daily'] }), true);
  assert.deepEqual(resumePartnerDraft(draft, null), { acceptable: ['throughout', 'daily'], beat: 3 });
  assert.deepEqual(toggleAcceptable('daily', draft.acceptable), ['throughout']);
  assert.deepEqual(resumePartnerDraft({ ...draft, partnerChoicesExplicit: undefined }, null), { acceptable: [], beat: 1 });
  assert.deepEqual(resumePartnerDraft({ ...draft, answer: 'neutral', acceptable: ['neutral'], partnerChoicesExplicit: undefined }, 'neutral'), { acceptable: ['neutral'], beat: 3 });
