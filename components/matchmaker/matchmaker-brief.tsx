...
export function MatchmakerBriefCard({ brief, loading, busy = false, error, onUpdate, onUndo, onRetry }: Props) {
  ...
  return (
    <View style={styles.card} accessibilityLabel="What the matchmaker understands about you">
      <Pressable accessibilityRole="button" accessibilityState={{ expanded }} accessibilityLabel={expanded ? 'Collapse your match brief' : 'Review and edit your match brief'} onPress={() => setExpanded((current) => !current)} style={({ pressed }) => [styles.cardHeader, pressed && styles.pressed]}>
